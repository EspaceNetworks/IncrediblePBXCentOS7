...
if [ -e "/root/incrediblepbx-install-log.txt" ]; then
 mv /root/incrediblepbx-install-log.txt /root/incrediblepbx-install-log-phase1.txt
fi
...
if [[ "$release" = "7" ]]; then
 if [[ "$arch64" = "true" ]]; then
  wget http://incrediblepbx.com/rpmforge-release-0.5.3-1.el7.rf.x86_64.rpm
 else
  wget http://incrediblepbx.com/rpmforge-release-0.5.3-1.el6.rf.i686.rpm
 fi
else
 if [[ "$arch64" = "true" ]]; then
  wget http://incrediblepbx.com/rpmforge-release-0.5.3-1.el6.rf.x86_64.rpm
 else
  wget http://incrediblepbx.com/rpmforge-release-0.5.3-1.el6.rf.i686.rpm
 fi
fi
...
if [[ "$release" = "6" ]]; then
 ...
 rpm -Uvh https://dl.fedoraproject.org/pub/epel/6/x86_64/libresample-0.1.3-12.el6.x86_64.rpm
 rpm -Uvh https://dl.fedoraproject.org/pub/epel/6/x86_64/libresample-devel-0.1.3-12.el6.x86_64.rpm
 rpm -Uvh http://mirror.pnl.gov/epel/7/x86_64/e/epel-release-7-7.noarch.rpm
...
then
 if [ ! -f /root/DMI ]; then
  ...
  echo "If install does not automatically resume after reboot,"
  echo "log in as root and run ./IncrediblePBX13-12R.sh again."
...
make distclean
# these next two don't work with Asterisk 13.8
#autoconf
#./bootstrap.sh
...
if [[ "$release" = "7" ]]; then
 ...
 menuselect/menuselect --disable app_mysql --disable app_setcallerid --disable func_audiohookinherit --disable res_fax_spandsp --disable test_named_lock --disable test_res_pjsip_scheduler menuselect.makeopts
...
# patch to use latest GPL repo for Incredible PBX GUI
